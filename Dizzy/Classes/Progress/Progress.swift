...
open class Progress {
    ...
    open var delegete: ProgressDelegete?
    ...
    /** get sharedInstance class of Progress */
    public static func sharedInstance() -> Progress {
        let iPHUD = Progress()
        return iPHUD
    }

    /** Attach the Progress in views */
    open func attachProgress(toViews: UIView...) {
        for view in toViews {
            let reinit = self.copy()
            reinit.view = view
            reinit.setupProgress(view: view)
            view.progress = reinit
        }
    }

    /** Attach the Progress in array views */
    open func attachProgress(toViews: [UIView]) {
        for view in toViews {
            let reinit = self.copy()
            reinit.view = view
    ...
    fileprivate func setupProgress(view: UIView) {
        ...
        if isBlurBox {
            ProgressUtilities.blurEffect(view: boxView, corner: boxCorner)
            self.boxView.alpha = 1
            self.boxView.backgroundColor = .clear
        }
        if isBlurModal {
            ProgressUtilities.blurEffect(view: modalView, corner: 0)
        ...
        // Fix: - Center Screen No Matter What
        boxView.addSubviewScreenCenter()
        // Issue: - Center Screen
    ...
    fileprivate func indicatorSetting() {
        ...
        let indSize = ProgressUtilities.getHeightPercent(view: boxView, percent: self.indicatorSize)
    ...
    fileprivate func boxSetting(view: UIView) {
        ...
        if ProgressUtilities.getWidthPercent(view: view, percent: 100) < ProgressUtilities.getHeightPercent(view: view, percent: 100) {
            boxWidth = ProgressUtilities.getWidthPercent(view: view, percent: self.boxSize)
            if iprogressStyle == ProgressStyles.vertical {
                boxHeight = ProgressUtilities.getWidthPercent(view: view, percent: self.boxSize)
            } else if iprogressStyle == ProgressStyles.horizontal {
                boxHeight = ProgressUtilities.getWidthPercent(view: view, percent: self.boxSize / 3)
            }
        } else {
            boxWidth = ProgressUtilities.getHeightPercent(view: view, percent: self.boxSize)
            if iprogressStyle == ProgressStyles.vertical {
                boxHeight = ProgressUtilities.getHeightPercent(view: view, percent: self.boxSize)
            } else if iprogressStyle == ProgressStyles.horizontal {
                boxHeight = ProgressUtilities.getHeightPercent(view: view, percent: self.boxSize / 3)
            }
        }
    ...
    fileprivate func captionSetting() {
        ...
        if iprogressStyle == ProgressStyles.vertical {
            ...
            let preferredSize = ProgressUtilities.getSizeUILabel(label: captionView)
